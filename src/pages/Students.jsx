...
const blankStudent = () => ({
  ...
  address:'', enrollDate: today(), monthlyFee:'', annualFee:'', status:'Active', notes:'',
})
        ...
        {d.id ? (
          <div>
            <DocumentUpload
              entityType="student"
              entityId={d.id}
              pathPrefix={`students/${d.admissionNo || d.id}`}
          ...
          <div style={{ fontSize:12, color: C.muted, background: C.hover, padding:'10px 12px', borderRadius:8 }}>
            Save the student first, then reopen to upload documents (Aadhaar, birth certificate, photo).
          </div>
        )}
        ...
        <div style={{ display:'flex', gap:8 }}>
          <Btn variant="ghost" small onClick={async () => {
            ...
            exportCSV('students.csv', students, [
              ...
              { label:'Annual Fee', value:'annualFee' }, { label:'Status', value:'status' },
            ])
          }}>Export CSV</Btn>
